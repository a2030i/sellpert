...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!caller || !['admin', 'super_admin'].includes(caller.role))
      return json({ error: 'Forbidden — مطلوب صلاحية أدمن' }, 403)

    const body = await req.json()
    const {
      merchant_code, platform, data_date,
      total_sales, order_count,
      platform_fees = 0, ad_spend = 0, margin = 0,
      rows, // optional: array of multiple rows
    } = body

    const db = createClient(SUPABASE_URL, SERVICE_KEY)

    // ── Bulk mode: array of rows ──────────────────────────────────────────────
    if (Array.isArray(rows) && rows.length > 0) {
      ...
      // Log one sync_log per merchant+platform combo
      const combos = [...new Map(validated.map(r => [`${r.merchant_code}|${r.platform}`, r])).values()]
      for (const r of combos) {
        ...
        await db.from('sync_logs').insert({
          ...
          platform: r.platform,
          status: 'success',
          records_synced: validated.filter(v => v.merchant_code === r.merchant_code && v.platform === r.platform).length,
          started_at: now,
          finished_at: now,
          error_message: null,
        })
    ...
    // Record in sync_logs so merchant sees it in Integrations page
    const now = new Date().toISOString()
    await db.from('sync_logs').insert({
      merchant_code,
      platform,
