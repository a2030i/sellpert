-- Trendyol product performance exports contain discounts, not marketplace
-- commission. A discount reduces net revenue but must never be labelled or
-- subtracted a second time as a platform fee.
...
-- Correct existing derived rows without rebuilding unrelated merchant data.
UPDATE public.performance_data pd
SET platform_fees = 0
WHERE pd.platform = 'trendyol'
  AND COALESCE(pd.is_edited, false) = false
  AND EXISTS (
    SELECT 1 FROM public.product_performance_snapshots snapshot
    WHERE snapshot.merchant_code = pd.merchant_code
      AND snapshot.platform = pd.platform
      AND snapshot.snapshot_date = pd.data_date
  )
  AND NOT EXISTS (
    SELECT 1 FROM public.orders order_row
    WHERE order_row.merchant_code = pd.merchant_code
      AND order_row.platform = 'trendyol'
  );

COMMENT ON FUNCTION security.rebuild_performance_data(text) IS
  'Rebuilds daily performance with one sales source per platform. Trendyol product-report discounts are not platform commission.';
