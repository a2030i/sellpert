...
import { AlertTriangle, Boxes, CalendarDays, CheckCircle2, ChevronDown, Clock3, Layers3, Package, RefreshCw, ShoppingCart, TrendingDown, TrendingUp, WalletCards } from 'lucide-react'
...
export default function DashboardV2({ merchant, onNavigate }: { merchant:Merchant|null; onNavigate:(view:PhaseOneView)=>void }) {
  ...
  const pending = new Set(selected.filter(o => STATUS_PENDING.has(String(o.status).toLowerCase())).map(o => `${o.platform}:${o.order_id}`)).size
