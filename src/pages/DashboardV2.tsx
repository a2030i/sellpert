    ...
    <div className="db-page">
      ...
      <section className="db-panel db-chart-panel">
        ...
        {model.chart.length > 0 ? <div className="db-chart"><ResponsiveContainer width="100%" height="100%"><AreaChart data={model.chart} margin={{ top: 10, right: isMobile ? 0 : 12, left: isMobile ? -28 : 0, bottom: 0 }}>
          ...
          <CartesianGrid stroke="var(--border)" vertical={false} /><XAxis dataKey="date" tick={{ fill: 'var(--text3)', fontSize: 11 }} axisLine={false} tickLine={false} /><YAxis tick={{ fill: 'var(--text3)', fontSize: 11 }} axisLine={false} tickLine={false} tickFormatter={v => v >= 1000 ? `${(v / 1000).toFixed(v >= 10000 ? 0 : 1)}k` : Math.round(v).toString()} /><Tooltip formatter={(v) => money(Number(v || 0), 2)} contentStyle={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 10 }} /><Area type="monotone" dataKey="sales" name="المبيعات" stroke="#0f958c" strokeWidth={2.5} fill="url(#salesFill)" /><Area type="monotone" dataKey="net" name="الصافي" stroke="#21324f" strokeWidth={2} fill="transparent" strokeDasharray="5 4" />
