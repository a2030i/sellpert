...
import { BookOpen, ChevronLeft, Download, Link2, PackageSearch, Save, Search, Truck, Unlink, X } from 'lucide-react'
...
export default function ProductCatalog({ merchant }: { merchant:Merchant|null }) {
  ...
  function profitabilityFor(item:CatalogItem): ProductProfitability | null {
    if (platform==='all' || !item.id) return null
  ...
  async function saveShippingCost() {
    if (!merchant || platform==='all') return
  ...
  function exportCsv() {
    const financial=platform!=='all'
    const rows=financial
      ? [['المنتج','SKU','المنصة','سعر البيع','تكلفة المنتج','نسبة عمولة المنصة','قيمة عمولة المنصة شاملة الضريبة','الشحن شامل الضريبة','عمولة Sellpert حسب العقد','صافي المبلغ الواصل','صافي الربح','الجدوى'],...items.map(item=>{const value=profitabilityFor(item);return [item.name,item.sku||'',platformNames[platform]||platform,value?.salePrice??'',item.cost_price,value?.commissionRate??'',value?.commissionValue??'',value?.shippingCost??'',value?.sellpertCommissionValue??'',value?.netReceived??'',value?.netProfit??'',value?viabilityLabel(value.viability):'بيانات ناقصة']})]
  ...
  return <div className="product-catalog" dir="rtl">
    ...
    <section className="catalog-toolbar"><label className="catalog-search"><Search size={17}/><input value={search} onChange={e=>setSearch(e.target.value)} placeholder="البحث بالاسم أو SKU أو الباركود"/></label><select aria-label="اختيار منصة حساب الربحية" value={platform} onChange={e=>setPlatform(e.target.value)}><option value="all">كل المنصات — عرض الربط</option>{Object.entries(platformNames).slice(0,-1).map(([key,label])=><option key={key} value={key}>{label}</option>)}</select><select aria-label="تصفية حسب حالة الربط" value={status} onChange={e=>setStatus(e.target.value as typeof status)}><option value="all">كل الحالات</option><option value="linked">مرتبط</option><option value="review">يحتاج مراجعة</option><option value="unknown">غير معروف</option></select></section>
    {platform!=='all'?<section className="catalog-shipping"><div><Truck size={18}/><span><strong>تكلفة الشحن في {platformNames[platform]||platform}</strong><small>أدخل أسوأ تكلفة متوقعة للطلب الواحد، شاملة ضريبة القيمة المضافة.</small></span></div><label><span>الشحن شامل الضريبة</span><input aria-label="تكلفة الشحن شاملة الضريبة" type="number" min="0" step="0.01" value={shippingInput} onChange={event=>setShippingInput(event.target.value)}/><em>ر.س</em></label><button disabled={savingShipping} onClick={saveShippingCost}><Save size={15}/>{savingShipping?'جارٍ الحفظ':'حفظ'}</button></section>:<div className="catalog-finance-hint">اختر منصة من الفلتر لعرض جدوى التسعير وصافي المبلغ الواصل لكل منتج.</div>}
    {error?<div className="catalog-error">تعذر تحميل دليل المنتجات: {error}</div>:null}
    <section className={`catalog-table-wrap ${platform!=='all'?'financial':''}`} aria-busy={loading}><table className="catalog-table"><thead>{platform==='all'?<tr><th>المنتج الموحد</th><th>المعرّفات المرتبطة</th><th>حالة الربط</th><th>إجمالي المخزون</th><th></th></tr>:<tr><th>المنتج الموحد</th><th>سعر البيع</th><th>تكلفة المنتج</th><th>نسبة عمولة المنصة</th><th>قيمة عمولة المنصة<br/><small>شاملة الضريبة</small></th><th>تكلفة الشحن<br/><small>شاملة الضريبة</small></th><th>عمولة Sellpert<br/><small>حسب العقد</small></th><th>صافي المبلغ الواصل</th><th>جدوى التسعير<br/><small>في أسوأ الأحوال</small></th><th></th></tr>}</thead><tbody>{!loading&&items.map((item,index)=>{const financial=profitabilityFor(item);return <tr key={item.id||item.mappings[0]?.id||index} className={selected===item?'selected':''} onClick={()=>setSelected(item)}><td><div className="catalog-product-cell">{item.image_url?<img src={item.image_url} alt=""/>:<span className="catalog-placeholder"><PackageSearch size={19}/></span>}<div><strong>{item.name||'منتج بلا اسم'}</strong><small>{item.sku||item.barcode||'—'}</small></div></div></td>{platform==='all'?<><td><div className="catalog-mappings">{item.mappings.length?item.mappings.map(m=><span key={m.id}><b>{platformNames[m.platform]||m.platform}</b>{m.identifier_value}</span>):<span>لا توجد معرّفات</span>}</div></td><td><Status value={item.match_status}/></td><td><strong className="latin-number">{Number(item.inventory).toLocaleString('en-US')}</strong></td></>:<><MoneyCell value={financial?.salePrice}/><MoneyCell value={item.cost_price}/><td><strong className="latin-number">{financial?.commissionRate!=null?`${formatNumber(financial.commissionRate)}%`:'—'}</strong></td><MoneyCell value={financial?.commissionValue}/><MoneyCell value={financial?.shippingCost}/><MoneyCell value={financial?.sellpertCommissionValue}/><MoneyCell value={financial?.netReceived} strong tone={financial?.netReceived!=null&&financial.netReceived<0?'danger':undefined}/><td><Viability value={financial}/></td></>}<td><button className="catalog-row-action" aria-label={`عرض ${item.name}`}><ChevronLeft size={17}/></button></td></tr>})}</tbody></table>
