    ...
    <div style={{ ...S.tableCard, padding: 18, marginBottom: 16 }}>
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 16, flexWrap: 'wrap' }}>
        <div style={{ display: 'flex', alignItems: 'flex-start', gap: 11 }}>
          ...
          <div>
            <strong style={{ display: 'block', fontSize: 15 }}>ربط التجار بحساب Omniful المركزي</strong>
            <span style={{ display: 'block', marginTop: 4, color: 'var(--text3)', fontSize: 11, lineHeight: 1.7 }}>احفظ Seller ID وStore ID لكل قناة. التوكن المركزي يبقى سرًا في الخادم ولا يظهر هنا.</span>
          </div>
        </div>
      ...
      <div style={{ display: 'flex', gap: 7, alignItems: 'flex-start', marginTop: 14, padding: '10px 12px', borderRadius: 9, background: 'var(--success-bg)', color: 'var(--success-text)', fontSize: 10, lineHeight: 1.7 }}>
        ...
        <span>كل قناة تُعزل بمعرّفها داخل Omniful قبل أي مزامنة، وتبقى المصادر الحالية للتاجر فعالة أثناء وضع التجربة.</span>
      </div>
    </div>
